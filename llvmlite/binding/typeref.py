...
class TypeRef(ffi.ObjectRef):
    ...
    def struct_num_elements(self):
        """
        Returns the number of elements in a struct type. When the type is
        not a struct, raises exception.
        """
        if not self.is_struct:
            raise ValueError("Type {} has no elements".format(self))
        return ffi.lib.LLVMPY_GetStructNumElements(self)

    def struct_element_type(self, n):
        """
        Returns the nth type in a struct type. When the type is not a
        struct, raises exception.
        """
        if not self.is_struct:
            raise ValueError("Type {} has no elements".format(self))
        assert n < self.struct_num_elements, "Invalid type index: {}".format(n)
